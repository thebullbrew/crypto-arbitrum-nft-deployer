...
A professional, script-driven toolkit for deploying an ERC-721 NFT collection to Arbitrum One — from smart contract to deployment to minting. Built on [Hardhat](https://hardhat.org), [OpenZeppelin Contracts](https://openzeppelin.com/contracts), and [viem](https://viem.sh).
...
- **Testnet-first workflow** — rehearse on Arbitrum Sepolia before touching mainnet
- **CI** — GitHub Actions compiles contracts and TypeScript on every push

## Prerequisites

- Node.js 18+
- A Arbitrum One RPC endpoint (dedicated provider recommended: Alchemy, Infura, QuickNode)
- Deployer wallet private key, plus ETH for gas
...
> ⚠️ **Always test on Arbitrum Sepolia first.** Point `RPC_URL` at a Arbitrum Sepolia endpoint and fund a throwaway deployer wallet before running against Arbitrum One mainnet.
...
├── hardhat.config.ts        # Arbitrum One network (chain id 42161)
├── .env.example
...
Deploys `BullBrewNFT` with your collection name, symbol, max supply, per-wallet limit, mint price, and base URI. Prints the contract address and a Arbiscan link — save the address as `CONTRACT_ADDRESS` in `.env`.

To verify the contract on Arbiscan, use these constructor arguments in order: `COLLECTION_NAME`, `COLLECTION_SYMBOL`, `MAX_SUPPLY`, `MAX_PER_WALLET`, `MINT_PRICE` (in wei), `BASE_URI`.
...
|---|---|
| `RPC_URL` | Arbitrum One RPC endpoint (Arbitrum Sepolia for testing, mainnet for launch) |
...
| `MINT_PRICE` | Price per mint in ETH (decimal string, e.g. "0.05") |
...
- Test the full flow on Arbitrum Sepolia before mainnet.
- Verify the deployed contract on Arbiscan before announcing a mint.
